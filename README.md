...
Точные версии `electron` и `better-sqlite3` зафиксированы в `package.json` без `^`
(мера риска R1: плавающая версия любого из них меняет ABI нативного модуля, и
сборка ломается там, где её никто не ждёт). Прогон `npm test` добавлен и в
Windows-джобу CI — модуль должен не только собраться, но и загрузиться под
win32-ABI; тест `tests/db/paths.test.ts` (риск R2) проверяет весь дисковый слой —
создание БД, миграции, `VACUUM INTO`-бэкап, копию на флешку и восстановление —
на пути с русскими буквами и пробелами, как в `%APPDATA%\Расписание колледжа`
у пользователя с русским именем профиля.

Не сделано: два реальных скриншота SmartScreen (заглушки в
`docs/windows-install.md` помечены `TODO`, что и как снимать — в
`docs/img/README.md`), ручная проверка на живой Windows 11 —
установка/обновление поверх/удаление без
