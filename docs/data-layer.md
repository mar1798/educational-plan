...
Здесь — как принципы работы с данными выглядят в коде и что легко сломать.
...
- Архивация и удаление — разные вещи и живут рядом: архив прячет запись из рабочих списков,
  удаление убирает её физически. Удаление доступно везде, где сущность можно завести:
  справочники, квалификации, учебные планы (вместе со строками и раскладкой), версии шаблона
  недели (вместе с сеткой), резервные копии. Составные удаления идут внутри `runOperation` —
  и потому отменяются одной кнопкой на экране «Операции».
...
— дешёвая страховка от программной ошибки:
...
именно поэтому занятие не привязано к одной группе (потоковые лекции, решения п. 23).
